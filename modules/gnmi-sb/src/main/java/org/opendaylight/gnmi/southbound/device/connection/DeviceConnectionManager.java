...
public class DeviceConnectionManager implements AutoCloseable {
    ...
    private ListenableFuture<Void> createMountPoint(final Node node, final DeviceConnection deviceConnection,
        ...
        final ListenableFuture<Void> mountPointFuture = Futures.transformAsync(readCapabilitiesFuture,
            capabilityResponse -> {
                ...
                try {
                    ...
                    // FIXME (GNMI-29): committed outside the claim lock (the commit blocks), so a delete racing
                    // this merge can leave capabilities behind; serialize per-node on the async path.
        ...
        // A failure after READY (capabilities RPC, missing JSON_IETF, schema) would leak the session and listener
        // and resurrect the deleted node; close on failure like the success branch. Cancellation (abort) is left as is.
