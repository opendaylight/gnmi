...
public class GnmiConnectionStatusListener implements AutoCloseable {
    ...
    public synchronized FluentFuture<CommitInfo> copyDeviceStatusReadyToDatastore()
        ...
        if (ConnectivityState.READY.equals(currentState)) {
            // Only this hand-off write can outlive the listener, so it alone checks config presence.
            // On a read failure proceed anyway: the device is mounted, and "could not tell" is not "deleted".
            boolean present = true;
            try {
                present = DatastoreUtils.nodePresentInConfig(dataBroker, nodeId);
            } catch (ExecutionException | TimeoutException e) {
                LOG.warn("Unable to read configuration of node {}, proceeding with READY write",
                        nodeId.getValue(), e);
            } catch (InterruptedException e) {
                Thread.currentThread().interrupt();
                LOG.warn("Interrupted while reading configuration of node {}, proceeding with READY write",
                        nodeId.getValue(), e);
            }
            if (!present) {
