...
public class DeviceConnectionInitializer implements AutoCloseable {
    ...
    class SessionInitializationHolder implements AutoCloseable {
        ...
        public void close() throws Exception {
            ...
            // Listener is closed before sessionProvider (see DeviceConnection#close for why), and every
            // step below must run even if an earlier one throws; the first failure is rethrown with any
            // later ones attached as suppressed, rather than one masking another.Exception failure = null;
            ...
            try {
                // SessionProvider#close() is only declared to throw Exception via AutoCloseable.
