...
containing [GnmiDataBroker](src/main/java/org/opendaylight/gnmi/southbound/mountpoint/broker/GnmiDataBroker.java), which is
...
```
    <dependency>
        <groupId>org.opendaylight.gnmi.modules.gnmi.southbound</groupId>
...
This example shows how to programmatically connect lighty gNMI-sb with a gNMI device. First we start lighty.io gNMI, then lighty.io controller and the gNMI device. For the device in this example, [lighty.io gNMI Device Simulator](../odl-gnmi-device-simulator/README.md) is used.

Creating a device connection and CRUD operation, is performed by writing data directly to the MD-SAL data store **without using RESTCONF**. The full example can be found inside the test - [GnmiWithoutRestconfTest](../odl-gnmi-test/src/test/java/org/opendaylight/modules/gnmi/test/gnmi/GnmiWithoutRestconfTest.java).
...
6. Set data modeled by [gnmi-test-model](../odl-gnmi-test/src/test/resources/models/plugin_models/gnmi-test-model.yang) to gNMI device
```java
...
```
Thefull example can be found inside the [GnmiWithoutRestconfTest](../odl-gnmi-test/src/test/java/org/opendaylight/modules/gnmi/test/gnmi/GnmiWithoutRestconfTest.java).
